...
import { getAllChoreDefinitions, CHORE_CATEGORIES } from "../lib/chores.js";
...
function GroupCard({
  ...
  const memberIds = useMemo(() => new Set(group.members.map(m => m.choreId)), [group.members]);
  const memberChores = useMemo(
    () => allChores.filter(c => memberIds.has(c.id)),
    [allChores, memberIds]
  );
  ...
  return (
    <li className="bg-surface border border-line">
          ...
          {memberChores.length === 0 ? (
            ...
            <ul className="m-0 p-0 list-none flex flex-col gap-px bg-line">
              {memberChores.map((c) => (
                <li
                  key={c.id}
                  className="bg-surface px-3 py-2 flex items-center gap-3"
                >
                  <div className="flex-1 min-w-0">
                  ...
                  <button
                    onClick={() => removeFromThisGroup(c.id)}
                    aria-label={`Remove ${c.title}`}
                    title="Remove from group"
                    className="text-muted hover:text-warn p-1 cursor-pointer bg-transparent border-0 shrink-0"
                  >
                    <X size={13} />
                  </button>
                </li>
